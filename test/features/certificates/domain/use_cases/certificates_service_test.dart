...
void main() {
  ...
  group('CertificatesService', () {
    ...
    test('init loads certificates from storage', () async {
      when(() => mockStorage.getCertificates()).thenAnswer((_) async => [certItem]);
      
      await service.init();
      
      expect(service.trustedCertificates.length, 1);
      expect(service.trustedCertificates.first.host, 'test.com');
    ...
    test('ignoreCertificate saves a new ignored certificate', () async {
      ...
      expect(service.ignoredCertificates.first.host, 'test.com');
