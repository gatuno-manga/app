...
- **API Configuration:**
  - Removed compile-time `API_BASE_URL` from `--dart-define`.
  ...
  - Added `REFERER` environment variable to centrally manage the `Referer` header for all network requests, defaulting to `https://gatuno.canto.internal/`.
- **Routing & Navigation:**
  ...
  - Moved the "Profile" view (`/users/me`) to be a sub-route accessible from the Settings page.
